...
export default function TransactionInspector({ transaction }: { transaction: Transaction | null }) {
  if (!transaction) {
    return (
      <div className="rounded-card bg-surface border border-border p-6 flex flex-col gap-1 shadow-card">
  ...
  return (
    <div className="rounded-card bg-surface border border-border p-6 flex flex-col gap-5 shadow-card reveal">
      <div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {transaction.neighbors.map((n) => (
            <span
              key={n}
              className="text-xs bg-selected-soft text-ink rounded-full px-2.5 py-1 border border-selected-border font-medium"
            >
              #{n}
            </span>
          ))}
